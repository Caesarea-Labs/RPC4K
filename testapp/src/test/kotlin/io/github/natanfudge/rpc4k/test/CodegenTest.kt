...
import io.github.natanfudge.rpc4k.generated.*
...
fun main(): Unit = runBlocking {
    startServer()
    val client = MyApi.client(OkHttpRpcClient("http://localhost:8080"), JsonFormat())
...
private fun startServer() = embeddedServer(CIO, 8080) {
    ...
    routing {
        post("/") {
            val server = MyApi.server(api, JsonFormat(), KtorSingleRouteRpcServer(call))
