...
internal class Rpc4kProcessor(private val env: SymbolProcessorEnvironment) : SymbolProcessor {
    ...
    private fun SymbolProcessorEnvironment.generateRpc(apiClass: KSClassDeclaration, resolver: Resolver) {
        val time = measureTimeMillis {
            ...
            // ApiDefinitionToClientInterface.convert(api).writeTo(codeGenerator, false, files)
            ApiDefinitionToNetworkClient.convert(api).writeTo(codeGenerator, false, files)
